...
@interface MMTableViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)setupDataSource
{
    ...
    NSDictionary *JSONDic =@{@"group":
                                 @[
                                     @{@"groupName":@"分组1",@"groupCount":@"3",@"groupArray":@[
                                     ...
                                     @{@"groupName":@"分组2",@"groupCount":@"5",@"groupArray":
                                           @[
                                     ...
                                     @{@"groupName":@"分组3",@"groupCount":@"3",@"groupArray":
                                           @[
                                     ...
                                     @{@"groupName":@"大分组4",@"groupCount":@"4",@"groupArray":
                                           @[
                                     ...
                                     @{@"groupName":@"分组5",@"groupCount":@"3",@"groupArray":
                                           @[
                                     ...
                                     @{@"groupName":@"分组6",@"groupCount":@"3",@"groupArray":
                                           @[
...
- (void)setupUI
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"MMCell" bundle:nil] forCellReuseIdentifier:@"MMCell"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    return model.isOpened ? model.groupFriends.count : 0 ;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    MMCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MMCell"];
    GroupModel *model = self.dataArr[indexPath.section];
    NSDictionary *dic = model.groupFriends[indexPath.row];
    ...
    cell.shuoshuoLabel.text = dic[@"shuoshuo"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (groupModel.isOpened) {
        ...
        CGAffineTransform newTransform = CGAffineTransformRotate(currentTransform, M_PI_2);//在现在基础上旋转指定角度
...
{
    GroupModel *groupModel = self.dataArr[sender.tag];
    UIImageView *imageV = objc_getAssociatedObject(sender, buttonKey);
    if (groupModel.isOpened) {
        [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationOptionTransitionNone animations:^{
            CGAffineTransform currentTransform = imageV.transform;
